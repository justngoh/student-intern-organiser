...
@app.route('/project_distribution')
def project_distribution():
    ...
    # Get current intake
    cursor.execute('SELECT name FROM Intakes WHERE status = "current"')
    current_intake = cursor.fetchone()[0]
    
    # Query to get project distribution
    query = '''
        SELECT 
            project,
            SUM(CASE WHEN course = 'Science' THEN 1 ELSE 0 END) as science_count,
            SUM(CASE WHEN course = 'Engineering and IT' THEN 1 ELSE 0 END) as engineering_count
        FROM Students 
    ...
    science_counts = [row[1] for row in results]
    engineering_counts = [row[2] for row in results]
    
    conn.close()
    
    return render_template(
        'project_distribution.html',
        projects=projects,
